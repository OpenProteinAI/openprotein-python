...
# force_recompute tests for svd_fit_post

def test_svd_fit_post_force_recompute_true(mock_session: MagicMock):
    ...
    api.svd_fit_post(mock_session, "model-1", sequences=["ACGT"], force_recompute=True)
    mock_session.post.assert_called_once()
    _, kwargs = mock_session.post.call_args
    assert kwargs["params"] == {"force": "true"}


def test_svd_fit_post_force_recompute_default(mock_session: MagicMock):
...
# force_recompute tests for svd_embed_post

def test_svd_embed_post_force_recompute_true(mock_session: MagicMock):
    """force_recompute=True should pass params={"force": "true"} to session.post."""
    mock_session.post.return_value.json.return_value = {
        "job_id": "embed-123",
    ...
    api.svd_embed_post(mock_session, "svd-1", ["ACGT"], force_recompute=True)
    mock_session.post.assert_called_once()
    _, kwargs = mock_session.post.call_args
    assert kwargs["params"] == {"force": "true"}


def test_svd_embed_post_force_recompute_default(mock_session: MagicMock):
    """force_recompute defaults to False, so params should be None."""
    mock_session.post.return_value.json.return_value = {
        "job_id": "embed-123",
        "job_type": JobType.svd_embed.value,
    ...
    api.svd_embed_post(mock_session, "svd-1", ["ACGT"])
    mock_session.post.assert_called_once()
    _, kwargs = mock_session.post.call_args
    assert kwargs.get("params") is None
