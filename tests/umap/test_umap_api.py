...
# force_recompute tests for umap_fit_post

def test_umap_fit_post_force_recompute_true(mock_session: MagicMock):
    """force_recompute=True should pass params={"force": "true"} to session.post."""
    mock_session.post.return_value.json.return_value = {
        "job_id": "job-123",
        "job_type": JobType.umap_fit.value,
        "status": "SUCCESS",
...
def test_umap_fit_post_force_recompute_default(mock_session: MagicMock):
    """force_recompute defaults to False, so params should be None."""
    mock_session.post.return_value.json.return_value = {
        "job_id": "job-123",
        "job_type": JobType.umap_fit.value,
        "status": "SUCCESS",
    ...
    api.umap_fit_post(mock_session, "model-1", "PLM", sequences=["ACGT"])
    mock_session.post.assert_called_once()
    _, kwargs = mock_session.post.call_args
    assert kwargs.get("params") is None


# force_recompute tests for umap_embed_post

def test_umap_embed_post_force_recompute_true(mock_session: MagicMock):
    """force_recompute=True should pass params={"force": "true"} to session.post."""
    mock_session.post.return_value.json.return_value = {
        "job_id": "embed-123",
        "job_type": JobType.umap_embed.value,
        "status": "SUCCESS",
        "created_date": "2023-01-01T00:00:00",
    }
    api.umap_embed_post(mock_session, "umap-1", ["ACGT"], force_recompute=True)
    mock_session.post.assert_called_once()
    _, kwargs = mock_session.post.call_args
    assert kwargs["params"] == {"force": "true"}


def test_umap_embed_post_force_recompute_default(mock_session: MagicMock):
    """force_recompute defaults to False, so params should be None."""
    mock_session.post.return_value.json.return_value = {
        "job_id": "embed-123",
        "job_type": JobType.umap_embed.value,
    ...
    api.umap_embed_post(mock_session, "umap-1", ["ACGT"])
    mock_session.post.assert_called_once()
    _, kwargs = mock_session.post.call_args
    assert kwargs.get("params") is None
